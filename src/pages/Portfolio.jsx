...
export default function Portfolio() {
    const portfolio = projects.map((project) => (
        <div key={project.id} className="my-8 lg:flex overflow-hidden bg-white dark:bg-gray-800 rounded-lg shadow-lg">
            <Link to={`/portfolio/${project.id}`} aria-label={`View details for the ${project.title} project`} className="lg:flex w-full items-center text-center lg:text-left">
                <div className="lg:w-1/2">
                    <img src={project.image} alt={project.name} className="w-full h-auto lg:w-4/5 mx-auto rounded-lg m-4 shadow-2xl lg:rounded-none lg:rounded-l-lg dark:shadow-none" />
                </div>
                <div className="lg:w-1/2 p-4 lg:p-8">
                    ...
                    <i className="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700 transition duration-150 ease-in-out dark:bg-blue-600 dark:hover:bg-blue-800">{project.buttonText}</i>
                </div>
            </Link>
        </div>
    ));
